...
public class MainActivity extends Activity {
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		marksRenderer.addMark(new Mark("MyMark", Geodetic3D
				.fromDegrees(0, 0, 0), AltitudeMode.ABSOLUTE, 0));
		marksRenderer
				.addMark(new Mark(
						new URL(
								"http://upload.wikimedia.org/wikipedia/commons/thumb/5/5a/Wikipedia's_W.svg/50px-Wikipedia's_W.svg.png"),
						Geodetic3D.fromDegrees(50, 0, 0),
			...
			public void run(final G3MContext context) {
				IBufferDownloadListener listener = new IBufferDownloadListener() {
					...
					public void onDownload(URL url, final IByteBuffer buffer,
							boolean expired) {
						context.getThreadUtils().invokeAsyncTask(
								new GAsyncTask() {
									...
									public void runInBackground(
											G3MContext context) {
										IJSONParser parser = context
												.getJSONParser();
										JSONBaseObject data = parser
												.parse(buffer);
										JSONObject object = data.asObject();
										JSONArray features = object
												.getAsArray("features");
										for (int i = 0; i < features.size(); i++) {
											JSONObject feature = features
													.getAsObject(i);
											JSONObject properties = feature
													.getAsObject("properties");
											String name = properties
													.getAsString("name", "");
											double population = properties
													.getAsNumber("population",
															0);
											JSONObject geometry = feature
													.getAsObject("geometry");
											JSONArray coordinates = geometry
													.getAsArray("coordinates");
											double lat = coordinates
													.getAsNumber(1, 0);
											double lon = coordinates
													.getAsNumber(0, 0);
											names.add(name);
											coords.add(Geodetic3D.fromDegrees(
													lat, lon, 0));
											pops.add(population);
										}
									}
				...
				long id = context.getDownloader().requestBuffer(
						new URL("file:///cities.geojson"), 1,
						TimeInterval.fromDays(30), true, listener, true);
